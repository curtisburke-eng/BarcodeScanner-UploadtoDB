...
#barCodeDeviceString = "Scanner  Scanner in FS Mode" # barcode device name
            ...
            if event.type == ecodes.EV_KEY:
                ...
                if data.keystate == 1 and data.scancode != 42: # Catch only keydown, and not Enter
                    ...
                    if data.scancode == 28: # if enter detected print barcode value and then clear it                       
                        ...
                        barcodeBuffer = functions.addToBuffer(barcodeBuffer,barcode);
                        #print("Buffer: " + barcodeBuffer)
                        
                        validLength = functions.checkBuffer(barcodeBuffer);

                        if (validLength):
                            db = MySQLdb.connect("192.168.1.61", "pi", "IESControls!", "BBDrawTrailers")
                            functions.processBuffer(db, barcodeBuffer);
                            barcodeBuffer = ""
